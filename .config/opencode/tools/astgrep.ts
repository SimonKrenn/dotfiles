...
function uniqueFiles(matches: AstGrepMatch[]): string[] {
  return [...new Set(matches.map((m) => m.file).filter((v): v is string => Boolean(v)))]
}
...
function withCommonSearchFlags(args: {
  ...
  const cmd = ["run", "--pattern", args.pattern, "--json=stream", "--color", "never"]
...
export const find = tool({
  ...
  args: {
    ...
    language: tool.schema.string().optional().describe("Language, e.g. ts, js, lua, py"),
    strictness: tool.schema
      .enum(STRICTNESS_VALUES)
      .optional()
      .describe("Pattern strictness (default ast-grep behavior)"),
    path: tool.schema.string().optional().describe("Path to search, defaults to current directory"),
    globs: tool.schema.array(tool.schema.string()).optional().describe("Optional include/exclude globs"),
  ...
  async execute(args, context) {
    ...
    if (!result.success) {
      return {
        ok: false,
        error: result.stderr || result.stdout || `ast-grep failed with exit code ${result.exitCode}`,
      }
    }
...
export const rewrite = tool({
  ...
  args: {
    ...
    strictness: tool.schema
      ...
      .optional()
      .describe("Pattern strictness (default ast-grep behavior)"),
    path: tool.schema.string().optional().describe("Path to rewrite, defaults to current directory"),
    globs: tool.schema.array(tool.schema.string()).optional().describe("Optional include/exclude globs"),
    apply: tool.schema
      .boolean()
      .optional()
  ...
  async execute(args, context) {
    ...
    if (!before.success) {
      return {
        ok: false,
        error: before.stderr || before.stdout || `ast-grep failed with exit code ${before.exitCode}`,
      }
    }
    ...
    const rewriteCmd = ["run", "--pattern", args.pattern, "--rewrite", args.replacement, "--update-all", "--color", "never"]
    ...
    if (!rewriteResult.success) {
      return {
        ok: false,
        error:
          rewriteResult.stderr || rewriteResult.stdout || `ast-grep rewrite failed with exit code ${rewriteResult.exitCode}`,
      }
    }
    ...
    const remainingMatches = after.success ? parseJsonStream(after.stdout).length : null
...
export const rule_find = tool({
  ...
  async execute(args, context) {
    const maxResults = args.maxResults ?? 50
    const cwd = context.directory || context.worktree
    const cmd = ["scan", "--inline-rules", args.yaml, "--json=stream", "--color", "never"]
    for (const glob of args.globs ?? []) cmd.push("--globs", glob)
    cmd.push(args.path ?? ".")
    ...
    if (!result.success) {
      return {
        ok: false,
        error: result.stderr || result.stdout || `ast-grep scan failed with exit code ${result.exitCode}`,
      }
    }
...
export const debug_query = tool({
  ...
  async execute(args, context) {
    ...
    if (!result.success) {
      return {
        ok: false,
        error: result.stderr || result.stdout || `ast-grep debug failed with exit code ${result.exitCode}`,
      }
    }
    ...
    return {
      ok: true,
      format,
      output: text.length > 12000 ? `${text.slice(0, 12000)}\n...truncated...` : text,
    }
  },
})
